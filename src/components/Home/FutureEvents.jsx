...
const FutureEvents = ({ setShowModal, showModal }) => {
  ...
  useEffect(() => {
    ...
    try {
        // Add multiple keywords separated by a comma
        const url = `https://images-api.nasa.gov/search?q=future`;
        ...
        // Filter items to only include those with a video or image
        items = items.filter(item => item.data[0].media_type === 'video' || item.data[0].media_type === 'image');
  ...
  return (
    <div className="max-w-3xl mx-auto flex flex-col items-center justify-center text-white pb-10 pt-10">
      <div>
        <h2 className="pt-20 pb-3 text-center bg-clip-text bg-gradient-to-r from-cyan-500 to-blue-500 text-5xl font-bold tracking-tight text-transparent">
          Future Events
        </h2>
        <p className="text-center text-white text-base md:text-2xl font-normal max-w-2xl pb-5 mx-auto">
          Things to look forward to in the future! Click on the add to calendar
          button to add the event to your Google Calendar.
        </p>
      </div>
      <div className="grid rounded-lg grid-cols-3 gap-2 md:grid-cols-3 max-w-3xl grid-auto-flow-dense">
          ...
          <div
            ...
            className={`${
              index % 10 === 0
                ...
                ? "col-span-1 row-span-1"
                : "col-span-1 row-span-1"
            } bg-gray-800 rounded-lg shadow-xl transform hover:translate-y-2 hover:brightness-150 transition-all duration-300 flex flex-col relative`}
          >
            ...
            <div
              ...
              className="absolute inset-0 rounded-lg bg-black opacity-40"
            ></div>
            <div
              onClick={() => {
                setSelectedItem(item);
    ...
    >
              ...
              <button
                className={`text-xs font-sans uppercase w-8 h-10 transition-all duration-300 ${
      ...
      <button
        ...
        className="mt-6 p-1 w-full buttonMain text-2xl"
      >
        Load More
      </button>
      <div>
